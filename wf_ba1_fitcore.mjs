...
function fixtures() {
  ...
  {   // スタート地点だけ閉じた小部屋。**【BD4・2026-09-21】意図して凍結した写しである。**
      ...
      //   **【BE5・2026-09-25】揃えられるかを測って、揃えないと決めた。** wf_roomfixture.mjs は部屋を原点に置くので、
      //   枠（2.0→0.503）だけでなく部屋の平行移動（(0.2, 0.84)→原点）も伴う。部屋基準の座標でスポーン 6 点を比べると、
      //   枠だけの差は本ゲートの 3 領域×4 carScale と卓上 cs 0.8/0.6（BD4 の倍率の残り）の 14 セルすべてで厳密に 0 だが、
      //   平行移動を含む差は 0 でない（本ゲートの 12 セルでは最大 1.11e-16 m で厳密一致は 6 点中 0〜1 点・minClearance は
      //   14 セルすべてで変わる／卓上 cs0.8 では 0.220 m）。揃える条件「全倍率で厳密に 0」を満たさないので、この写しと PINNED は変えない。
