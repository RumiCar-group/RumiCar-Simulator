...
2. 「🌐 GitHubで共有」ボタンを押す → **コース JSON のダウンロードが始まり**、続けて
   GitHub の[アップロード画面](https://github.com/RumiCar-group/RumiCar/upload/master/courses/community)が開きます
3. ダウンロードしたファイルをその画面に置いて (ドラッグ＆ドロップ) コミットするとプルリクエストが
   作成されます (書込権限が無い場合は GitHub 側で fork を作る案内が出ます)
4. マージ後、全利用者のメニューに表示されます

> **なぜ 2 段階なのか**: 以前はコースの中身を URL に載せて GitHub の新規ファイル作成画面へ
> 渡していましたが、URL の長さには上限があり (実測で約 6,600 文字)、壁が 24 本を超えると
> 投稿できませんでした。実際のコースは数万〜数十万文字になるため、**中身を URL から外して**
> ファイルとして渡す方式に変えています。コースの大きさに関係なく投稿できます。
>
> **ファイル名は置く前に変えられます。** シミュレータが付ける名前は ASCII 英数しか使えないため、
> 日本語のコース名は \`course-<数字>.json\` になります。投稿コースの一覧に出る表示名は
> ファイル名ではなく JSON の \`name\` (英語表示では \`name_en\` があればそれ) なので、ファイル名は
> 管理しやすい半角英数の名前に変えてからアップロードしてください (例: \`racing-course.json\`)。
>
> ブラウザがポップアップを塞いだ場合は、シミュレータの画面下のログに投稿ページの URL が出ます。
> それを自分で開いて、ダウンロードしたファイルを置いてください。
