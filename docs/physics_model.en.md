...
> **Primary data**: `wf_ao8_gonogo.mjs` (20 cells), `wf_ap14_wallhairpin.mjs` (10 wall-bounded cells × 2 drivetrains plus grip's dynamic arm), `wf_ap15_downhill_gonogo.mjs` (72 cells) and `wf_drift_reexam.mjs` (16 free-space cells, the switching optimisation and the two-car runs) **print their tables directly** — run `node wf_<name>.mjs` (`--full` for a wider sweep). Continuous steering and the recovery boundary (29°/36°) are **pinned as regression assertions** by `wf_as12_steer.mjs` (that one prints pass/fail only, not the numbers).
