...
- **Braking mid-corner (trail braking) — what can and cannot be claimed**: measured over **two independent grids (54 and 288 cells)** sweeping radius, entry speed, brake duration, road μ and steering type.
  ...
  - **FF and AWD cars *lose* recovery cells — unlike FR.** Over the 288-cell grid, **FF loses 18 and gains 5** (283 → 270) and **AWD loses 15 and gains 8** (258 → 251). Only FR "never gets worse in a single cell": **the trade this equipment makes reverses with the drivetrain.**
...
- **⚠ On the tabletop and mid-scale, this equipment makes braking *worse* — the sign flips with the regime.** For the same FR car, braking gets **2.11× stronger at full scale** but **0.77× weaker on the tabletop** and 0.77× at mid-scale. The cause is not the regime as such but the **semi-implicit wheel ODE** that is enabled on the tabletop and mid-scale for speed (§13.2, v4.0.0). Its damping term uses the **linear-region tire stiffness**, a deliberately safe-side (over-large) value, so a wheel cannot build up slip fast enough and **delivers less force to the road than it was commanded**. That is equally true of the default motor brake (delivered/commanded = 0.59); spreading the command thinly over four wheels only makes the shortfall bite harder (0.43). **The discriminating quantity is whether spreading the command gets *more* of it onto the road**, and the standing gate machine-checks across all 8 conditions that "the equipment helps ⟺ the delivered/commanded ratio rises". Where the semi-implicit path is not taken (low-grip surfaces, slip tires, full scale) the ratio is 0.96–2.11, i.e. **any loss stays within 4%**. Improving the semi-implicit approximation itself would change the default tabletop physics and is therefore out of scope for this version; it is **recorded as a carry-forward**.
