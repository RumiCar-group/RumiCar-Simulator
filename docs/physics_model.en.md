...
### Can a leader defend *position* in a chase-style (multi-car, point-to-point) race? — measuring blocking tactics (Stage AX, measured on engine v8.0.0, 2026-09)

**This is a different question from the tables above.** The go/no-go tables measure **single-car lap/section times**, and the item just above (Stage AU3) measures **lap times of two cars on a closed circuit**. In a touge-style race — **start and finish in different places**, with **2–6 cars running at once** — winning is decided by **finishing order**, not by time. Whether "the leader loses time, but the chasers lose more, so the leader wins" actually holds cannot be answered by a time measurement. So we swapped the metric to finishing order and measured again.

**Conditions of the measurement (this section's conclusion is scoped to them)**: **tabletop scale × the precise v2 engine** (a production configuration you can pick in the "physics engine" selector — but note the **tabletop default is the dynamics model**, so this is *not* what you get by leaving the defaults alone), all cars normal FR, 6 touge courses × **4 road-width levels** (as shipped / walls moved in to 4.5, 3.0 and 2.0 car widths), 4 leader tactics (`race` = simply take the fastest line, `inside` = occupy the innermost *reachable* position, `sideways` = turn sideways in corners, `drift` = lock the rear axle on entry to break the attitude), fields of 2/3/4/6 cars, and chasers that run at 1.0/1.15/1.35× the leader's pace as an **omniscient script** (it reads its own and the leader's position and attitude directly — i.e. a strong chaser, stacked against the blocker). Full sweep: **10,368 races**.

**Answer: blocking tactics do not defend position.** Not one of the four tactics raised the leader's win rate significantly above `race` in any condition — **0 of 66** (the 66 decidable conditions = 6 touge × 4 widths × 3 tactics, minus the 6 where the lines are identical and nothing can be decided; **still 0 when the tactic's wins are restricted to races with no contact and no close approach** — a conservative count that demands cleanliness of the tactic side only, since demanding it of both sides measures a different quantity, namely "the tactic's wins are tidier"). Pairs won by a tactic alone: **14**. Pairs won by `race` alone: **1,366** (the tactic side is 1.0%).

| Road width | Room for two cars abreast | Leader win rate `race` | `inside` | `sideways` | `drift` |
|---|---|---|---|---|---|
| As shipped (6.6–11 car widths) | +0.37–0.72 m | **85%** | 69% | 13% | 64% |
| 4.5 car widths | +0.200 m | **85%** | 83% | 19% | 85% |
| 3.0 car widths | +0.080 m | **100%** | 100% | 93% | 86% |
| 2.0 car widths | **0 m** | **100%** | 100% (＝`race`) | 89% | 100% |

(These win rates average **three chaser pace ratios, 1.0/1.15/1.35**. At the shipped width the leader is **never once passed** even by a 1.15× chaser; passing happens only at 1.35×, and even there in 46% of races. So the 85% at the shipped width includes arms in which the leader cannot be passed at all.)

**What defends position is the road width, not the tactic.** The `race` win rate is 85% as shipped → 85% at 4.5 car widths (flat) → 100% at 3.0 car widths → 100% at 2.0: **narrower than 3.0 car widths and it reaches 100%**. At 2.0 car widths (road 0.16 m, car 0.08 m) the room for two cars abreast is **exactly 0**, so overtaking is geometrically impossible. At that level all four tactics collapse onto the centre line (the distinction between tactics disappears), so what decides the result there is **geometry, not tactics**. That is precisely why touge chase ("cat and mouse") is run as a two-car format in the real world — the road does not allow passing — and this is a direct confirmation of it.

**Why occupying the inside does not work.** First we measured **the passing speed at each lateral position** across all 20 corners of the touge courses (dividing the road width into 7 positions per corner and deciding reachability by **actually driving it**, not by a geometric formula). We found that (a) the fastest lateral position lies in the **inner band in 19 of 20 corners** (the outer band is fastest in 0 corners) — as expected — but (b) **93.2%** of reachable points can be passed at the car's top speed and the fastest position was flat out in 19 of 20 corners, so a "fast line" is not fast, it is **short**; (c) of the 60 points in the inner band (20 corners × 3 inner subdivisions), **23 are unreachable at all** (16 wall, 6 steering-limited, 1 recovery) — so **the blocker cannot take the inside either** in those corners; and (d) even when one car width is parked on the innermost reachable position, **0 corners become impassable** — the chaser's corner section merely gets **+17.9%** slower on average (max +35.5%), which is **+3.2–4.1%** over a full lap.

What actually happened in the races follows from that: the hairpins of touge 1/2/3 have radii right at the minimum turning radius, so the reachable corridor narrows to a single point and **`race` and `inside` go through the corner in the same place** (in-corner inside occupancy 0.44 vs 0.43). The only difference is **whether the car hugs the next corner's side on the straight** (0.05 vs 0.44) — and hugging one side leaves the other side open, so **the pass happens on the straight**. So "even if the outside is open, blocking the inside counts as blocking it" did **not** hold under these conditions.

**"The leader loses, but the chasers lose more" did not happen.** The self-cost of each tactic (how much the leader's own finishing time degrades, averaged over all conditions) was `inside` **+0.24 s**, `drift` **+0.97 s**, `sideways` **+9.44 s** — and that cost translated straight into lost positions. `sideways` in particular gets passed while the leader is stopped, and on the narrow levels the leader even retires (115 DNFs for `sideways` and 72 for `drift` across the full sweep). The prediction that "a tactic which sheds speed loses twice over, because it also lengthens its own path" was **not refuted** — but what we measured is finishing time only: we did **not** decompose the cost into "the speed given up" and "the extra path length", and we make no claim about that split.

> **Primary data**: `wf_ax1_touge_base.mjs` (acceptance of the touge reference driver, about 30 s), `wf_ax2_lines.mjs` (reachability and passing speed per lateral position, about 32 s), `wf_ax3_block.mjs` (this section). The default run of `wf_ax3_block.mjs` is a reduced sweep of 1,152 races (about 4 minutes); **every number in this section comes from `--full`** (10,368 races, about 36 minutes). `node wf_ax3_block.mjs --json` prints the tables as JSON. The **18 narrowed variant courses** are at the end of the course list ([4.5/3/2-Car Width]).
> **How far this conclusion reaches**: the chasers are **omniscient scripts** (i.e. a strong chaser, stacked against the blocker), so **two programs that drive on ToF alone will not necessarily show the same gap**. Field size (2–6) barely moved the win rate (`race` 92–93%); what decided finishing order was **the road width and the pace difference to the chaser** (at the shipped width the leader is never passed even by a 1.15× chaser; only at 1.35× is it passed, in 46% of races).
### Can a corner be built that *only* drift can take? — a bench reverse-engineered from the steering floor (Stage AY, measured on engine v8.1.1, 2026-09; only the finisher counts taken when the courses were added are on v8.2.0)

**This is yet another question, different from the three above.** The go/no-go table (AO8) measured **single-car lap time**; the section after it (AU3) measured **lap times of two cars on a circuit**; the one after that (AX) measured **finishing order in a point-to-point race**. All three ask whether drift is **faster**. This one asks whether it can **get through at all** — can we *build* a corner that steering cannot physically take, so that sliding is the only option left, and if we can, does sliding actually get through it?

**One inequality governs the whole thing.**

> When the corridor's **outer radius** `R_out` falls below the **minimum turning radius** `R_min`, steering cannot take the corner at all.
> `R_min = wheelbase / tan(max steering angle)` is a **kinematic floor that does not depend on speed**: on the tabletop it is **0.2920 m** (= 0.130 m / tan 24°); mid-scale 0.584 m, full scale 5.840 m. **Slowing down does not get you inside it** (the same point we drew out in v8.1.1: braking cannot reach it).
> Here `R_out = R (radius of the intended line) + usable`, with `usable = road width/2 − car width/2` — i.e. **the outermost concentric circle the car's reference point (rear-axle centre) can reach**.

Across the 20 shipped touge corners and the 60 derived-touge corners, **not one** corner meets that condition (0 under all three definitions of `R_out`; the closest is Fictional Touge: Long Winding (Steep Slope) [2-Car Width] at 0.3172 m = **1.086×** `R_min`). So **the reason no "drift is mandatory" regime ever showed up in earlier measurements is not that drift lost — it is that no such stage existed in the courses.** We therefore reverse-engineered the stage: outer-radius ratio `R_out/R_min` at **1.02 / 0.95 / 0.856 / 0.80**, road width at **0.16 m (2.0 car widths) / 0.28 m (3.5 car widths)**, heading change 180° — **8 cells**.

**Answer 1: there is no regime where drift is *mandatory*.** In the 12 cells steering cannot take (6 on the default surface plus 6 on low-μ), neither an optimiser with no constraint on how much it slides, nor a **"forced-depth" optimiser that actually slid to \|β\| 90° and 41° of rear-axle slip angle** (67°/22° across the 6 default-surface cells, 90°/41° across the 6 low-μ ones), ever produced a run that stayed inside the corridor: **zero clean solutions**. Even the closest miss still leaves the corridor by **0.0345 m**. So: **a corridor steering cannot take cannot be taken by sliding either.** This held when we changed the random seed and when we doubled the search budget.

**Answer 2: no regime where drift is *advantageous* was found under these conditions.** GO (combined time ratio ≤ 0.98 *and* 3/3 clean under ±10% entry speed) came out **0/8**, and **0/8** again on the low-μ surface. **But we do not write "there is none"** — change the random seed and the combined time ratio spreads over **0.932 to 1.168**, straddling the 0.98 threshold (in **2 of the 4 rows** where a comparison existed, the speed condition flips with the seed; with the main seeds **GO is 0**, and across the other seeds **0 of 12 are GO** while **2 of 12** land at a time ratio ≤ 0.98). What we can say is only "**not found with these seeds and this budget**". **Answer 1 (mandatory) is solid; answer 2 (advantageous) is not** — please do not merge the two.

**Answer 3: if a win exists, it is confined to cells grip can also take.** Every row whose time ratio moved with the seed in answer 2 was a cell steering could take. So the drift story stays, all the way down, "**steering could take it, but sliding might be faster**" and never "**steering cannot take it, so slide**". This re-reading also **applies to what we wrote before**: the AO8/AU3 benches (R5/6.5/8/50/120 m, full-scale `R_min` = 5.840 m) sit at outer-radius ratios of **1.23 / 1.49 / 1.83 / 9.45 / 21.61** — **all five above the floor**. So the "one window in free space" at R5 means **sliding was 11.9% faster through a corner steering could also take**, not that sliding was the only way through.

**The boundary where steering stops working is not one line but two.** The value depends on which assumption you read it under, so **always state the assumption**.

| Assumption | Condition | Analytic prediction | Measured (bisection) |
|---|---|---|---|
| The entry **lateral position can be chosen** (you may enter anywhere across the corridor) | `R + usable ≥ R_min` | `R_out/R_min` = 1.000 | **1.0040** (0.16 m width) / **1.0025** (0.28 m) |
| You enter **down the middle** of the corridor | `R + usable/2 ≥ R_min` | 1.0685 / 1.1712 | **1.0728** / **1.1739** |

(The second formula is **specific to a 180° heading change**. Without a choice of offset, the circle driven at the steering floor has its centre displaced from the intended line's centre by `R_min − R`, so the maximum distance from that centre is `2·R_min − R`. Re-derive it if you change the angle.)

**That `R_out` is a statement about a point path — the car's body makes it stricter.** The outer front corner swings wider than the reference point's circle, and on the tabletop that swing-out is **0.03654 m**. At a road width of 0.16 m the `usable` margin is only 0.0400 m, so **the swing-out alone eats 91% of it**. The body-aware ratio `(R_out − swing-out)/R_min` is **below 1 in all 8 cells** (0.675 to 0.895), so even the two cells whose outer-radius ratio is 1.02 — above 1 — become "**the reference point can be threaded through, the body cannot**". Sweeping a rigid car against the actual walls confirms it: the best steady-arc line (β = 0, `R ≥ R_min`) fits in **none of the 7 cells that exist as real walls**, missing by **0.0310 to 0.0951 m** (the 8th — ratio 0.80 at a 0.28 m width — is outside the sweep, because its inner wall radius goes negative and it does not exist as a real wall). The same sweep fits in 4 of 4 control cells (outer-radius ratio 1.5 / 2.0), so this is not a coarse sweep.

**The 7 of these 8 cells that exist as real walls are at the end of the course list** ([Steering-Limit Bench]; the one cell at outer-radius ratio 0.80 with a 0.28 m width is left out because its inner wall radius goes negative and it does not exist as a real wall). Measured when they were added (2026-09-08), the three built-in samples finish **1 of 21** across the 7 courses (the one that got through is outer-radius ratio 1.02 at 3.5 car widths — the side steering *can* take; engine = the tabletop default, the dynamic model; on precise v2 it is 0/21). **What is still unmeasured is the multi-point-turn line** — if anything gets through here, that is it.

> **Primary data**: `wf_ay1_rmin_bench.mjs` (this section; 37 assertions; about 225 s by default, and `--full` reaches the same conclusion) plus `wf_drift_opt.mjs`, the optimiser extracted out of it. `node wf_ay1_rmin_bench.mjs` prints the tables directly and `--json` emits JSON. **Measurement conditions**: tabletop × the **precise v2 engine** (the production setup you pick under "Physics engine" in the toolbar; note that **the tabletop default is the dynamic model**, so this is not what you get by just pressing start), car type `normal_fr`, all optional equipment at defaults, 180° heading change.
> **How to read a NO-GO**: again, this is **the best found within a finite search budget**. Answer 1 (no regime where drift is mandatory) was checked against both seeds and budget, but answer 2 (no advantageous regime) means "not found with these seeds and this budget" and is not a proof of non-existence.
...
- **Whether a slope holds the car or rolls it away while coasting is decided by rolling resistance:** in FREE the car runs on the net longitudinal acceleration formed by gravity `gFwd` together with a **Coulomb-type rolling resistance that never reverses the velocity** (magnitude capped at `CAR.coast`) — from the tentative speed `u + gFwd·dt` we subtract no more resistance than that speed itself. So **on a gentle slope with |gFwd| ≤ coast the car stays at rest** (resistance eats all of gravity), while **on a steep slope with |gFwd| > coast it rolls away on balance**. What the user sees: release the throttle and the car holds still on a gentle gradient but starts rolling by itself on a steep one. In **both the dynamics model and precise v2** the sub-steps are fine enough that the tiny increments of rolling away from rest fall below the stop-snap threshold `DYN.uStopHard` and would be crushed to zero on every sub-step, so **the stop snap is bypassed only under that steep-slope condition** (`gRoll`; the same condition exists in `physics_dyn.js` and in `physics_v2.js`). Flat ground (`gFwd = 0`) matches the previous formula exactly and is byte-identical. **The threshold equals `coast` itself in the dynamics model and precise v2** — classic (kinematic) carries the same `|gFwd| ≤ coast` expression, but it is composed with a per-step pull back toward a target speed of zero, so its **effective threshold is about 2×coast** (measured over a 3 s run: 0.060 m at a ratio of 2.0 — barely a crawl — and 2.86 m at 2.001). Implementation: the FREE branch in `physics_dyn.js` / `physics_v2.js` plus `gRoll`, and the FREE-at-rest branch in `physics.js`.
