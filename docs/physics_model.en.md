...
- **⚠ On the tabletop and mid-scale, this equipment makes braking *worse* — the sign flips with the regime.** For the same FR car, braking gets **2.11× stronger at full scale** but **0.77× weaker on the tabletop** and 0.77× at mid-scale. The cause is not the regime as such but the **semi-implicit wheel ODE** that is enabled on the tabletop and mid-scale for speed (§13.2, v4.0.0). The scheme freezes the ground-contact speed during the implicit update (an operator split), so a wheel's deceleration cannot keep up with the body's and it **delivers less force to the road than it was commanded** (v7.7.0 attributed this to the damping using the safe-side linear-region stiffness; the v7.8.0 measurement shows that is not the main cause — see the carry-forward below). That is equally true of the default motor brake (delivered/commanded = 0.59); spreading the command thinly over four wheels only makes the shortfall bite harder (0.43). **The discriminating quantity is whether spreading the command gets *more* of it onto the road**, and the standing gate machine-checks across all 8 conditions that "the equipment helps ⟺ the delivered/commanded ratio rises". Where the semi-implicit path is not taken (low-grip surfaces, slip tires, full scale) the ratio is 0.96–2.11, i.e. **any loss stays within 4%**. Improving the semi-implicit scheme itself would change the default tabletop v2 physics, so v7.8.0 still leaves it alone and **only measured the mechanism as a carry-forward** (the figures in (a)–(d) come from a one-off measurement harness that is not shipped in the repository): (a) moving the damping from the linear-region stiffness to the local slope does *not* fix it (friction/motor ratio 0.758 → 0.685, computed from the actual final speed; the 0.77 printed by chapter I of the standing gate uses the target final speed). (b) The real cause is **the operator split that freezes the ground-contact speed during the implicit wheel update**; the error scales with the substep width h (h·kD). (c) Conversely, **the current tabletop / mid-scale v2 brakes about 36% weaker on the tabletop and about 32% weaker at mid-scale than the reference solution with the motor brake** (decelerations computed from the actual final speed: 2.34 vs 3.65 and 2.49 vs 3.65 m/s²). The reference is the explicit integration used before v4.0.0 (substep cap 256); raising the cap to 4096 changes it by less than 0.1% (converged), and running the current semi-implicit scheme with 4096 substeps converges to the same value (3.65 m/s²) — the difference is purely splitting error. It is a quantity that was not part of the v4.0.0-era A/B characterization (terminal speed, steady turning radius, launch distance, βmax). (d) A naive fix — predicting the change of ground speed from the previous substep and integrating the slip variable implicitly — brings the motor brake back to within −5% of the reference, but **for four-wheel braking it turns into a period-2 numerical oscillation at the production substep width (8 substeps early in the stop: the body is pushed forward in 22% of substeps and the rear longitudinal force alternates sign in 83%), so it is not a valid measurement**. A real fix has to solve the wheel–body coupling within the same substep. The frozen benchmarks f0–f3 (tabletop dynamics, full-scale v2) and the official-race `verifyHash` (full-scale v2) never take this path, so fixing it would not move them, but **the reproducibility of tabletop / mid-scale v2 practice records would change**, so adopting it is a decision for a separate version.
