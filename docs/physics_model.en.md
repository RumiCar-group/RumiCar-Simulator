...
- **In a wall-bounded tight hairpin, grip's geometric best line does not fit in the first place.** A steady-arc geometric sweep fails in **8 of the 10 cells**, and those 8 split into two mechanisms: **6 cells** where the corridor's outer radius is below the minimum turning radius R_min (= 5.84 m), and **2 cells** where the outer radius clears R_min but the **car's own footprint** does not fit (a point could round it, but the body's outer front corner cuts the wall). Applying **active driving that carries a shallow slip (peak \|β\| of 12–23°)** to those cells, **3 of the 8 complete the full 180° without ever touching a wall** (rear-wheel drive FR only; AWD is **0/8** under the same sweep — the front wheels' drive torque kills the rotation before it develops). The margin is **fragile**, though: in 2 of those 3 cells only **0.3 mm / 4.7 mm** of clearance to the wall is left. And those 3 cells **never apply counter-steer at all** (the minimum **achieved** steering angle — where the servo actually pointed — is 0%; the *command* did swing to full opposite lock, so counter-steer must be read from the achieved angle). So what can be said today is only that **"where grip's steady arc will not fit, active driving that includes a shallow slip can sometimes get through"** — **not** that "rear-wheel drift is the only way through", because grip's own *transient* lines (using the straight sections) have not been measured yet.
- **Adding downhill gradient (5°/10°) does not change the conclusion.** Flat / 5° / 10° × 24 = **all 72 cells remain NO-GO** (the **smallest** drift/grip total-time ratio is 0.990, short of the 0.98 required for GO). What the gradient does change is the **mechanism**: on the **low-μ 180° hairpins**, FR runs out of its speed budget at every gradient and collapses into a **pivot** (rotation in place), whereas **AWD escapes that exhaustion on the 5°/10° descents** (at 10° the minimum speed during the slide rises an order of magnitude, 0.06–0.11 → **1.67–2.04 m/s**). It still does not become faster than grip.
- **The fact that the drift now "completes" is not a gradient effect — it comes from fixing the measurement driver.** With the counter-steer sign corrected, clean runs across the 72 cells went 116 → **145/216**. But the three **flat** / low-μ / 180° / FR cells that went 0/3 → 3/3 clean take **1.94–1.97×** grip's time, and their peak \|β\| is 90° — a **pivot**, i.e. the failure mode where the speed budget has run out. The honest phrasing is "**drift can now finish, and it is still slower**".
- **What is fast is the *shallow* slip.** The 3 wall-bounded cells above peak at \|β\| = 12–23°, whereas the deep-slip runs collapsed into a pivot and took about twice grip's time (even when they finished). **"The deeper you slide, the faster you go" does not appear anywhere in this simulator's measurements.**

> **Primary data**: `wf_ao8_gonogo.mjs` (20 cells), `wf_ap14_wallhairpin.mjs` (10 wall-bounded cells × 2 drivetrains) and `wf_ap15_downhill_gonogo.mjs` (72 cells) **print their tables directly** — run `node wf_<name>.mjs`. Continuous steering and the recovery boundary (29°/36°) are **pinned as regression assertions** by `wf_as12_steer.mjs` (that one prints pass/fail only, not the numbers).
