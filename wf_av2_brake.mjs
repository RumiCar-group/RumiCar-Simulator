...
// ══ I. 領域依存（**この装備の効果は領域で符号が変わる**）═══════════════════════════
// 【AV2 敵対的検証で追加】旧版のゲートは fullscale しか走らせておらず、**アプリの既定領域である
//   卓上を 1 度も検査していなかった**（AS9/AS11/AV1 は全領域を走らせている）。実測すると
//   **卓上/中スケールの既定タイヤでは装備が制動を弱める**（friction/motor = 0.75〜0.85）。
//
// 機序（特定済み）: 制動指令 bk とタイヤ容量の比が領域で桁違いに違う（卓上 0.50 / fullscale 2.87）
//   ことではなく、**AP13 の半陰的車輪 ODE（`siWheel`）が力を過小に伝える**ことによる。
//   【AV3 で機序を測定し直した】AV2 時点では「半陰的化の減衰 kD が線形域の剛性＝安全側で、輪がスリップを
//   立ち上げ切れない」と書いたが、kD を局所勾配へ寄せる変異では直らない（friction/motor 0.758→0.685）。
//   真因は **陰的更新の間に接地速度 vcx を凍結する作用素分割**（誤差 ∝ h·kD）。現行の卓上 v2 は参照解
//   （v4.0.0 以前の原 explicit・nSub 上限 256・上限 4096 でも 0.1% 以内で収束）より motor の制動が卓上で約 36%・
//   中スケールで約 32% 弱い。**本コメントの数値は実終速で計算した減速度**（この I 章が印字する比 0.768 は
//   目標終速 uEnd 基準＝別基準。両者の差は最終 tick の行き過ぎ 1〜3%）。接地速度の変化を前 substep から予測して
//   滑り変数で陰的化する素朴な変異は motor では参照解の −5% まで戻るが、4 輪制動では本番 nSub（序盤 8）で
//   周期 2 振動（制動中の ΣFx>0 が 22% の substep）になり測定として成立しない＝修正には車輪と車体の連成を同一
//   substep で解く形が要る。既定物理を変えるため AV3 でも実装せず申し送り（機序と数値は docs/physics_model.md
//   §13.16。測定は同梱していない一回限りのハーネスによる）。指令を 4 輪へ薄く配ると過小伝達がさらに効くこと自体は実測どおり。
//   ∴ **陽的経路（`siWheel=false`）では効果の符号が反転して装備が有利になる**（下の I1 が述語）。
//   これは AP13 の既存近似の性質で AV2 が作ったものではない（motor 単独でも実力/指令比は 0.58）。
console.log('\n=== I: 領域依存（装備の効果は領域で符号が変わる。機序＝AP13 半陰的車輪 ODE）===');
{
  ...
  ok(si.length >= 2 && si.every(r => r.ratio < 0.85),
     `I1 **半陰的経路（卓上/中スケールの既定タイヤ）では装備が制動を 15% 以上弱める**: ${si.length} 条件すべてで friction/motor < 0.85（実測 ${si.map(r => r.ratio.toFixed(3)).join(' / ')}）。**アプリの既定領域は卓上なので、これは利用者が最初に出会う挙動である**`);
  ok(ex.length >= 5 && ex.every(r => r.ratio > 0.95),
     `I2 **陽的経路では悪化しても 4% 以内で、ロックが起きる条件では 1.6〜2.1 倍に有利化する**: ${ex.length} 条件すべてで friction/motor > 0.95（実測 ${ex.map(r => r.ratio.toFixed(3)).join(' / ')}）⇒ 悪化の原因は「領域」ではなく **半陰的化そのもの**`);
  ok(si.every(r => r.dM < 0.8) && si.every(r => r.dF < r.dM),
     `I3 機序: 半陰的経路では **motor でも実力/指令 < 0.8**（${si.map(r => r.dM.toFixed(3)).join('/')}）＝過小伝達は AP13 の既存の性質。指令を 4 輪へ薄く配るとさらに下がる（${si.map(r => r.dF.toFixed(3)).join('/')}）`);
