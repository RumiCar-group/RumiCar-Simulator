...
//   半陰的化の減衰 `kD = μFz·C·Bp/(κP·denom)` は **線形域の剛性＝飽和域の実勾配を上回る安全側**
//   （physics_v2.js の AP13 コメントが自らそう書いている）で、そのぶん輪はスリップを立ち上げ切れず
//   指令より小さい力しか出さない。指令を 4 輪へ薄く配るとこの過小伝達がさらに効く。
//   ∴ **陽的経路（`siWheel=false`）では効果の符号が反転して装備が有利になる**（下の I1 が述語）。
//   これは AP13 の既存近似の性質で AV2 が作ったものではない（motor 単独でも実力/指令比は 0.58）。
//   **AV3 への申し送り**: kD を局所勾配へ寄せるかは既定物理を変えるので AV2 のスコープ外。
