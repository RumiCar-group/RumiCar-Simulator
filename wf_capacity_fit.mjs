...
//    ⚠【2026-09-12・Stage AZ5 の実測で 1 点を絞り込んだ】上の「対象外の 2 つ」のうち **noRace 側は現状
//      到達できない**: `normalizeCourse`(course.js) は `noRace` を写さないので**投稿/保存/編集コースは
//      noRace を持てず**、`noRace` を持つのは出荷の 2 本 (競技グラウンド / 競技サーキット) だけで、
//      その 18 セルを含む 1188 セルで capN=0 は 0 件 (`wf_az2_fitguard` の B)。∴ 残る到達経路は
//      **「既に卓上」側だけ**で、そこは ⑤ が `log.capZeroWarn` で正直に告知する (無言ではない)。
//      **この 2 つを同じ重みで書かないこと**——片方は測って到達不能と分かっている。
