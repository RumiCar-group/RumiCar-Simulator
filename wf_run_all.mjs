...
const GATES = [
  ...
  'wf_be6_intake.mjs',
  // BE7: 上限内でも重い投稿コースの計算量（BB-4 ①②③）。①櫛形で freeSpawn 1 回が出荷最大の条件の 2 倍以内（廊下 BFS の
  //   発見ノード上限が効くこと・見通し判定の候補限定が判定コアを速くしていること）②車体スケールの掃引でも壁配列ごとの
  //   グリッドが GRID_CACHE_MAX 個まで・ヒープがその分に収まる ③描画用のレイ照会＝全壁走査・扇と深度ビューと測距が BE7 前の
  //   経路と同じ出力・1 フレームぶんの計算が BE7 前の経路より十分速い・古い contact_v2.js で読み込める。変異で赤。
  //   実ブラウザのフレーム時間と追従表示の画素一致は browser/check_be7_frame.mjs。所要は本ホスト実測 約 75s（単独・2026-09-25）。
  'wf_be7_heavy.mjs',
].map((name) => ({ name, args: [] }));
