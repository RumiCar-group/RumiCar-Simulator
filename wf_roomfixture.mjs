...
//
// **統一しない 1 箇所（意図的）**: `wf_ba1_fitcore.mjs` の `fixtures()` にある `fx:pocket` は旧 az2 治具の写しだが、
//   同ゲートは**改修前ツリーで取った出力ダイジェストを凍結して突き合わせる**（`PINNED`・2026-09-15 取得）。
//   配置を変えると凍結値を刻み直すことになり、回帰記録としての価値が消える。∴ **そのまま凍結して残す**。
//   BE5（2026-09-25）で揃えられるかを測り直した: ここへ揃えると部屋の平行移動を伴い、同ゲートの格子の全セルで
//   スポーン 6 点（部屋基準）と minClearance が改修前と一致しなくなる（卓上 cs0.8 では下の ⚠ と同じ 0.220 m）ので、
//   揃えない判断を維持した（数値は同ファイルの注記）。
//
// 導出（呼ぶ 4 本で同一）:
