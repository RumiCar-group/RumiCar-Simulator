...
- **バイナリ資産ゼロ** — `public/` は JS 46件 + `index.html` + `style.css` + `data/courses.json` のみ。画像は1枚も使っておらず、車体・コース・エフェクトはすべて実行時に手続き的に描画されます
...
73本の常設ゲートが同梱されています。改変後の回帰確認に使ってください。
...
`wf_*.mjs` を足したら `wf_run_all.mjs` の `GATES`（常設ゲート）か `EXCLUDED`（理由つきの非実行）に登録してください。どちらにも無いファイル・実在しない名前・二重の登録があると、ゲートを回す前に exit 1 で止まります（`--list` でも同じ）。
