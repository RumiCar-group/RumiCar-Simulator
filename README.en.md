...
wf_*.mjs                  71 permanent verification gates (regression checks after changes)
...
- **Deterministic.** The same input reproduces the same result, byte for byte. 71 verification gates machine-check this invariance, guaranteeing the compatibility of race records and share URLs. The gates ship inside this repository, so **anyone can clone it and run `node wf_run_all.mjs` to verify with their own hands** (no dependency beyond Node.js).
