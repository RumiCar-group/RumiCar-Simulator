...
//   own=false (第三者) … ④⑤ 公式レースのコース解決 / ⑦ 投稿しようとする形。**他人へ渡る**ので
//                        投稿と同じ基準 (枠との包含まで) で見る。
//   own=true  (自分)   … ① エディタの JSON 取込 / ② 保存コースの選択 / ⑧ エディタの ✔適用。
//                        **拒否は「自分が作ったものを失う」**なので、事故 (構造不正・桁違いの大きさ・
//                        遠い壁) だけを止める。枠を縮めた自作コースは通す。
//   ⚠ ① は当初 own=false だったが 2026-09-18 に own へ改訂した (根拠は :2617 の注記。#edExport/#edSave が
//     無検査ゆえ「書き出せたのに読み戻せない」非対称が生じていた)。ここを直すときは :2617 も同時に見ること。
